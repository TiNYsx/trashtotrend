...
export default async function PostSurveyPage() {
  ...
  try {
    const result = await query(
      "SELECT id, question_en, question_th, question_type, options, display_order FROM post_survey_questions WHERE is_active = true ORDER BY display_order ASC"
    )
    if (result && result.rows) {
      postQuestions = result.rows.map((row: any) => ({
        ...row,
